...
THEORY ListVariablesX IS
  ...
  Local_List_Variables(Machine(Genealogie))==(Meres,Peres,Femmes,Hommes);
  List_Variables(Machine(Genealogie))==(Meres,Peres,Femmes,Hommes);
  External_List_Variables(Machine(Genealogie))==(Meres,Peres,Femmes,Hommes)
END
&
...
THEORY ListInvariantX IS
  ...
  List_Invariant(Machine(Genealogie))==(Hommes <: PERSONNES & Femmes <: PERSONNES & Hommes/\Femmes = {} & Meres: Hommes\/Femmes-H0-F0 --> Femmes & Peres: Hommes\/Femmes-H0-F0 --> Hommes)
END
&
...
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Machine(Genealogie))==(Hommes,Femmes,Meres,Peres:=H0,F0,{},{});
  Context_List_Initialisation(Machine(Genealogie))==(skip);
  List_Initialisation(Machine(Genealogie))==(Hommes:=H0 || Femmes:=F0 || Meres:={} || Peres:={})
END
&
...
THEORY ListSubstitutionX IS
  Expanded_List_Substitution(Machine(Genealogie),naitre)==(pere: Hommes & mere: Femmes | @bambin.(bambin: PERSONNES-Femmes-Hommes ==> (Hommes:=Hommes\/{bambin} [] Femmes:=Femmes\/{bambin} || Peres:=Peres<+{bambin|->pere} || Meres:=Meres<+{bambin|->mere})));
  List_Substitution(Machine(Genealogie),naitre)==(ANY bambin WHERE bambin: PERSONNES-Femmes-Hommes THEN CHOICE Hommes:=Hommes\/{bambin} OR Femmes:=Femmes\/{bambin} END || Peres(bambin):=pere || Meres(bambin):=mere END)
END
&
...
THEORY ListANYVarX IS
  List_ANY_Var(Machine(Genealogie),naitre)==(Var(bambin) == atype(PERSONNES,?,?))
END
&
THEORY ListOfIdsX IS
  List_Of_Ids(Machine(Genealogie)) == (H0,F0,PERSONNES | ? | Meres,Peres,Femmes,Hommes | ? | naitre | ? | ? | ? | Genealogie);
...
THEORY VariablesEnvX IS
  Variables(Machine(Genealogie)) == (Type(Meres) == Mvl(SetOf(atype(PERSONNES,?,?)*atype(PERSONNES,?,?)));Type(Peres) == Mvl(SetOf(atype(PERSONNES,?,?)*atype(PERSONNES,?,?)));Type(Femmes) == Mvl(SetOf(atype(PERSONNES,?,?)));Type(Hommes) == Mvl(SetOf(atype(PERSONNES,?,?))))
END
&

...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Machine(ExempleTD),incr)==(btrue | xx:=xx+1 [] (xx>yy+1 ==> yy:=yy+1 [] not(xx>yy+1) ==> xx:=xx+1));
  List_Substitution(Machine(ExempleTD),incr)==(CHOICE xx:=xx+1 OR IF xx>yy+1 THEN yy:=yy+1 ELSE xx:=xx+1 END END);

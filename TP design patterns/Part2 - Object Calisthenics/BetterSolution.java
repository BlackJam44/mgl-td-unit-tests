// https://github.com/CyberLemonade/Codingame/blob/master/Very%20Hard/Music%20Scores.java
import java.util.*;
import java.io.*;
class Solution {
    ...
    public static void main(String[] args) {
        ...
        String str = s.ans.substring(1);
        System.out.println(str);
    }

    /* ------- NOT TOO MUCH INDENTATION LEVELS --------- */

    void hasNextFunction(String next, int w, int h, boolean[][] grid, int bCount, int cntX, int cntY, int minX){
      char ch = next.charAt(0); //TYPE OF CHARACTER
      int lnt = h; //LENGTH
    ...
    void input() {
        ...
        while (in.hasNext()) {
          String next = in.next();
          hasNextFunction(next , w, h, grid, bCount, cntX, cntY, minX);
        }
    ...
    void function3(int start, int stop, int note, boolean found, String type, int i, int cnt){
      if (!found) {
          start = i;
          note = cnt;
          found = true;
    ...
    void function2(int start, int stop, int note, boolean found, String type, int i, int cnt){
      for (int k = 0; k < noteH-1; k++) {
          if (grid[i][cnt]) {
            function3(start, stop, note, found, type, i, cnt);
          }
          cnt++;
      }
    }

    void function1(int start, int stop, int note, boolean found, String type, int i, int cnt){
      for (int j = 0; j < 6; j++) {
          function2(start, stop, note, found, type, i, cnt);
          cnt+=ruleH+1;
      }
    }
    ...
    void getNotes() {
        ...
        for (int i = minX; i <= maxX; i++) {
            int cnt = minY;
            inner:
            function1(start, stop, note, found, type, i, cnt);
            if (found) {
                stop = i-1;
                findNote(note,type);
            }
